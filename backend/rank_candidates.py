...
def _upsert_rankings(ranked_list: list, job_id: str) -> None:
    ...
    # Upsert to avoid duplicate key errors if re-running
    supabase.table("resume_rankings").upsert(
        records,
        on_conflict="resume_id",  # primary key/unique constraint
        ignore_duplicates=False,
    ).execute()

    print(f"✅ Supabase: upserted {len(records)} rows into resume_rankings")
